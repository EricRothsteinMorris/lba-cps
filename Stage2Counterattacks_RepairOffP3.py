...
logging.basicConfig(filename='T2T3repaired_offP3.log', filemode='w', level=logging.DEBUG)
...
Ranges[OMV3]=ValveStates
Ranges[QP2]=PumpStates 
Ranges[OP3]=PumpStates 
...
class CPSState:
    ...
    def step(self,attack):
        ...
        if attack != None:
            self=attack.apply(self) #We are going to apply the attack by components, so you'd better define attack as a product function.
    ...
    def setComponent(self,component,transformation):
        ...
        if component==QMV2 :
            ...
            self.Stage3.o.MV3=transformation(self.Stage3.o.MV3)
        elif component==OP3 :
            self.Stage3.o.P3=transformation(self.Stage3.o.P3)
        elif component== YL2 :
...
def offP3IfP2OffAndLowlevel(s:CPSState):
    if lowT3levelY(s) and s.Stage2.x.P2==off: #Attack common cause
        s.Stage3.o.P3=off  #counter attack common action
    return s
labels[offP3IfP2OffAndLowlevel]='offP3IfP2OffAndLowlevel'
...
def LatentBehaviourAnalysis(Attacker,CounterAttacker,Requirements,TestingTime):
    ...
    AttackCounter=0
    repair=Transformation(offP3IfP2OffAndLowlevel,None,None)
    while True:
...
def createReport(AttackEnumeration,CounterAttackEnumeration,NormalBehaviour,UnsuccessfulAttackToBehaviour,SuccessfulAttackToBehaviour,AttackAndCounterAttackToBehaviour,TestingTime):
    ...
    with open('Stage2NormalBehaviourRepaired_offP3.csv','w', newline='') as csvfile:
    ...
    if len(UnsuccessfulAttackToBehaviour.keys())>0:
        with open('Stage2UnsuccessfulAttackBehaviourRepaired_offP3.csv','w', newline='') as csvfile:
            fieldnames=['Attack#']
    ...
    if len(SuccessfulAttackToBehaviour.keys())>0:
        with open('Stage2SuccessfulAttackBehaviourRepaired_offP3.csv','w', newline='') as csvfile:
            fieldnames=['Attack#']
            fieldnames+=list(range(TestingTime))
            # for k in AttackToBehaviour.keys():
            #     fieldnames+=list(AttackToBehaviour[k].keys())
        ...
        with open('Stage2AttackCounterattackBehaviourRepaired_offP3.csv','w', newline='') as csvfile:
    ...
    # with open('Stage3AttackBehaviour_offP3.csv','w', newline='') as csvfile:
    #     fieldnames=['A']
    #     ResultsWriter = csv.DictWriter(csvfile, fieldnames=fieldnames)
    return 0
...
def main():
    #CONFIG
    TestingTime=6*60*60# 6 hours
    # 4 requirements: tanks never overflow (2 reqs) and tanks never empty (2 reqs)
    ...
    #Coefficients for the attacker. This attacker overrides the output of the controller PLC2
    AttackerCoefficients=toConstValues({OP2:PumpStates, OMV2:[valve_open,valve_closed]}) 
    #This attacker has visibility over the real level of water tank 3. They act on P2, but see P3.
    ...
    #This counterattacker overrides the output of the controller PLC3
    CounterAttackerCoefficients=toConstValues({OMV3:[valve_closed,valve_open], OP3:[on,off]}) 
    #The visibility of this attacker is given by the sensor. Attacker has "better visibility"
    CounterAttackerBasis = set([Predicate(lowT3levelY, labels[lowT3levelY]),Predicate(goodT3levelY, labels[goodT3levelY]), Predicate(highT3levelY, labels[highT3levelY])]) 

    #This defines the attacker and the counterattacker as duals
    ...
    #Perform Latent Behaviour Analysis
