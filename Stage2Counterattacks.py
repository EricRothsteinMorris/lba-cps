...
logging.basicConfig(filename='T2T3original.log', filemode='w', level=logging.DEBUG)
...
class CPSState:
    def __init__(self,Stage2,Stage3,deformation):
        self.Stage2=Stage2
        self.Stage3=Stage3
        self.deformation = deformation
        self.time =0
...
def LatentBehaviourAnalysis(Attacker,CounterAttacker,Requirements,TestingTime):
    ...
    while True:
        ...
        if Attack==None:
            #we are done!
            break
        cps=CPSState(Stage2(None),Stage3(None),None)
        AttackCounter+=1
        ...
        if BrokenRequirement==None:
            ...
            while True:
                ...
                cps=CPSState(Stage2(None), Stage3(None),Attack)                
...
def createReport(AttackEnumeration,CounterAttackEnumeration,NormalBehaviour,UnsuccessfulAttackToBehaviour,SuccessfulAttackToBehaviour,AttackAndCounterAttackToBehaviour,TestingTime):
    ...
    with open('Stage2NormalBehaviourOriginal.csv','w', newline='') as csvfile:
    ...
    if len(UnsuccessfulAttackToBehaviour.keys())>0:
        with open('Stage2UnsuccessfulAttackBehaviourOriginal.csv','w', newline='') as csvfile:
            fieldnames=['Attack#']
    ...
    if len(SuccessfulAttackToBehaviour.keys())>0:
        with open('Stage2SuccessfulAttackBehaviourOriginal.csv','w', newline='') as csvfile:
            fieldnames=['Attack#']
            fieldnames+=list(range(TestingTime))
            # for k in AttackToBehaviour.keys():
            #     fieldnames+=list(AttackToBehaviour[k].keys())
        ...
        with open('Stage2AttackCounterattackBehaviourOriginal.csv','w', newline='') as csvfile:
